...
| `apps/web/e2e/hosted-forms.spec.ts` | Full real-MongoDB integration: publish + OG tags, required-field 422, malformed-email 422, successful 201, idempotent retry (same key → same id), distinct keys → distinct submissions, unpublish → 410 + unknown/unpublished slug → 404, republish with a new required field, and two builder-UI tests (Publish and follow the real link; Unpublish, reload, republish) | 10 |
